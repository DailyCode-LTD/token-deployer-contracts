...
describe("LiquidityGenrator Non ETH", function () {
  it("should deploy uniswap ", async function () {
    ...
    this.token = await SimpleToken.deploy(
      "TEST LP",
      "TLPT",
      DECIMALS,
      INITIAL_SUPPLY
  ...
  it("should have isWETH === true", async function () {
    expect(await this.liquidityGenerator.isWETH()).to.equal(false);
  });
  ...
  it("should fail to set marketing receiver to address zero", async function () {
    const [owner, lpReceipient, marketing, alice, bob, carol] =
      await ethers.getSigners();
    await expect(
      this.liquidityGenerator.setMarketingReceiver(ethers.constants.AddressZero)
    ).to.be.revertedWith("!zero address");
  });
  ...
  it("should create LP ", async function () {
    ...
    await this.token.approve(this.router.address, 1n * 10n ** DECIMALS);
    // create  LP
    /*
  ...
  it("should comply to taxes on transfer ", async function () {
    ...
    const ethBalanceOfMarketingBefore = await this.token.balanceOf(
      marketing.address
    );
    ...
    const ethBalanceOfMarketing = await this.token.balanceOf(marketing.address);

    expect(ethBalanceOfMarketing).to.be.greaterThan(
      ethBalanceOfMarketingBefore
    );
    expect(await this.lpPair.balanceOf(lpReceipient.address)).to.be.greaterThan(
      0
    );
  });
  ...
  it("should comply to taxed when buying", async function () {
    ...
    await this.token.transfer(alice.address, ethers.utils.parseEther("0.01"));
    ...
    await this.token
      .connect(alice)
      .approve(this.router.address, ethers.utils.parseEther("0.01"));
    await this.router
      .connect(alice)
      .swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ethers.utils.parseEther("0.01"),
        0,
        [this.token.address, this.liquidityGenerator.address],
        alice.address,
        ethers.constants.MaxUint256
      );
  ...
  it("should comply to taxed when selling", async function () {
    ...
    await this.router
      ...
      .swapExactTokensForTokensSupportingFeeOnTransferTokens(
        amount,
        0,
        [this.liquidityGenerator.address, this.token.address],
        alice.address,
        ethers.constants.MaxUint256
      );
